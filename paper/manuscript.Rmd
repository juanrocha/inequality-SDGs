...
The ambitious agenda is materialized in 169 targets and indicators, yet not all targets are monitored or properly measured, and not all countries report them, presenting significant data and knowledge gaps.
...
When the SDGs were adopted in 2015, work began to create a comprehensive database of SDG indicators [@schmidt-traub2017; @sachs2024], to be updated annually.
Based on these data, previous analyses found that goals associated with poverty alleviation, well-being, economic development, and innovation (e.g. SDGs 1, 3, 7, 8, 9) tended to be synergistic with other goals, while goals related to responsible consumption, climate action, natural resources, and cooperation showed the most trade-offs (e.g. SDGs 11, 12, 13, 14, 16, 17) [@pradhan2017; @s41599-019-0335-5].
Drawing upon relevant World Bank data, Lusseau and Mancini [@s41893-019-0231-4] find these patterns to be modulated by the overall income level of countries, with low-income countries showing synergies across all goals, while trade-offs start to appear in higher income categories.
More recently, [@s41467-023-44679-w] analysed transboundary SDG interactions and found that high income countries play a disproportionate role in influencing the achievement of SDGs in other countries.
...
Second, within-country inequality has been rising in the last decades even in high income countries, and as a consequence of the Covid pandemic, in between country inequality raised for first time in a generation.
...
And third, recent conceptual and theoretical work has proposed mechanisms by which increase in inequality can impact the environment, and changes in the environment can feedback into further inequalities [@hamann2018; @dade2022; @leach2018].

Recent work also shows that a trilemma exists where countries struggle to achieve high prosperity, high environmental standards while reducing inequality [@Wu_2024].
...
Countries trajectories on the trilemma space, however, suggest the hypothesis that social progress can be achieved without compromising the biosphere [@Wu_2024].

It remains an open question if these patterns hold across different datasets and scales.
...
The UN dataset offers time series from 1963 to 2025 for 17 SDGs, 168 targets, and 247 indicators, 687 time series, in 413 administrative areas (N = `r sdg |> nrow()` observations).
...
# dat is the most recent UN dataset 2025, keys is the var names
...
Missing values were then imputed using a cubic spline, leaving us with `r df_dat |> filter(!is.na(rptinc992j_p0p100)) |> pull(country_code) |> unique() |> length()` countries, `r df_dat |> pull(year) |> unique() |> length()` years of data across 9 indicators for the WB dataset; and `r keys |> filter(series %in% unique(dat$series)) |>  pull(series) |> unique() |> length()` series capturing `r keys |> filter(series %in% unique(dat$series)) |>  pull(indicator) |> unique() |> length()` indicators, `r dat |> pull(geoAreaName) |> unique() |> length()` countries over `r dat |> pull(timePeriodStart) |> unique() |> length()` years for the UN dataset.
...
```{r echo=FALSE}
# create table: source, series code, name, 
```
...
```{r table1, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE}

library(kableExtra)

un_vars <- df_un_reduced |> names() |> str_subset("^\\d{4}_") |> str_remove("\\d{4}_") |> unique()
un_vars <- un_vars[-c(7:9)]
wb_vars <- names(df_dat)[4:12]
iq_vars <-  names(df_dat)[c(14,16,17)]

tbl <- tibble(
    Source = c(rep("UN",length(un_vars)), rep("WB", length(wb_vars)), rep("WII",length(iq_vars))),
    Variable = c(un_vars, wb_vars, iq_vars)
    #Units = c(rep("Percentage", 4), "Thousands of hectares", "0-1")
)
...
tbl$Units[1:4] <- "Percentage"
tbl$Units[6] <- "0 to 1"
tbl$Units[16:18] <- "0 to 1"
tbl$Variable[16:18] <- c(
    "Ratio of pre-tax national income for working adults (rptinc992j_p0p100)", "Gini coefficient (gptinc992j_p0p100)", "Share of the top 1% (sptinc992j_p99p100)"
)


knitr::kable(tbl, format = "latex", booktabs = TRUE, caption = "Summary of variables used") %>%

...
author: |
    | \small Juan C. Rocha^1^, Tong Wu^2^, Kevin Berry^3^, Tomas Chaigneau^4^,  Maike Hamann^4^, Emilie Lindkvist^1^, Jiangxiao Qiu^6^, 
    | \small Caroline Schill^1,7^, Alon Shepon^8^, Anne-Sophie Crépin^7^, and Carl Folke^1,7^
    | \small
    | \footnotesize ^1^Stockholm Resilience Centre, Stockholm University
    | \footnotesize ^2^The Natural Capital Project and Institute for International Studies, Stanford University
    | \footnotesize ^3^College of Business and Public Policy, University of Alaska Anchorage
    | \footnotesize ^4^Environment and Sustainability Institute, University of Exeter
    | \footnotesize ^5^Center for Complex Systems in Transition, Stellenbosch University
    | \footnotesize ^6^School of Forest Resources & Conservation, Fort Lauderdale Research and Education Center, University of Florida
    | \footnotesize ^7^Beijer Institute of Ecological Economics, The Royal Swedish Academy of Sciences
    | \footnotesize ^8^Department of Environmental Studies, The Porter School of the Environment and Earth Science, Tel Aviv University
...
Target: EcolEcon?
/ People and Nature?
/ World Development / PNAS / ERL / NatSust?
[4000w]

Authors so far (patterns team): Juan, Maike, Tong, Jiangxiao, Anne-Sophie, Dineke.
The paper is open to all BYS, currently listing the same co-authors as the trilemma paper.
You are welcome to join but you need to read, improve and approve the submission (aim to submit by Sept 2025 before Mistra deadline so we can cite it as pre- print).
Please provide your comments before August 22th.

Alternative suggested titles:

-   Global SDG assessment for just biosphere stewardship

-   Trade-offs between biosphere goals and inequality at the national scale

\begin{abstract}

  The relationship between inequality and the biosphere has been hypothesized to have feedbacks, to be mutually dependent. If that is true, one could expect that such feedbacks (nonlinear dynamics) give rise to inequality regimes and potential tipping points between them. Here we explore synergies and trade-offs between inequality and biosphere related sustainable development goals. Using ordination methods we confirm the existence of inequality regimes and find a potential candidate of tipping point for corruption, one of the drivers of inequality.
...
These differences suggest that it may be worthwhile investigating if the nature of the mechanisms linking SDG to each other could be dependent on income level and its distribution.
...
A Hartigan's Dip test for unimodality results on significant p-values for all variables except the Gini in wealth (ratio of income p \< 2.2e-16, ratio of wealth p = 0.020, share of 1% wealth p \< 2.2e-16, Gini of wealth p = 0.113, share of 1% income p \< 2.2e-16, Gini of income p = 9.023e-05).
...
Previous work on the origins of inequality have proposed mechanisms across scales, but they are not necessarily comparable at the national level.
For example, micro level dynamics such as aspirations, conspicuous consumption, social norms, or perceptions of fairness have been proposed as a potential mechanism creating inequality or disincentivising cooperation [@hamann2018].
At the meso-level, market-concentration or lobbying have been proposed as mechanism by which powerful classes tend to favor institutions that further enable capital accumulation [@hamann2018; @chancel2020unsustainable].
...
Another key mechanism proposed is corruption [@holmberg2009; @scheve2017], which similar to market concentration, enable actors in power to seek their individual interests at expenses of the social good.

\begin{figure*}[ht]
\centering
\includegraphics[width = 6.5in, height = 2in]{fig_histeresis}
\caption{\textbf{Inequality, corruption, and potential hysteresis} We find two inequality regimes that correspond to the country groups previously reported (A). Corruption has been proposed as a generative mechanism of inequality. Here we find support for the idea of corruption being a driver (B) that can tip countries between inequality regimes. For values below 35 or above 80 in the corruption index only one regime exist, in between these values two regimes of inequality exist. A regression analysis shows that negative trends on corruption index (lower values means more corruption) increases inequality measured as the Gini coefficient on income (C).}
\label{fig:hysteresis}
\end{figure*}

Using data from the quality of government datset [@holmberg2009], here we find support for the hypothesis that corruption increases inequality.
A linear regression shows that one need different slopes for the country groups found in the clustering analysis, and the higher the corruption trend (negative and significant coefficient), the higher the inequality measured as the mean Gini on income (Fig \ref{fig:hysteresis}).
...
In other words, there is a region on the corruption space where the two inequality regimes co-exist (approximately between 35 and 80).
There are not many transitions between one regime and the other in the historical record, thus we cannot empirically confirm the existence of a tipping point in corruption that potentially trigger shifts in inequality regimes.
...
Although nearly all countries have committed to achieving progress on SDGs by 2030, it is now clear that at the global level most of these goals won’t be met.
...
Furthermore, it is clear that socio-economic inequalities can hinder collective action and other types of political activity conducive for environmental stewardship.
...
The lack of variability in the SDGs datasets and a large proportion of missing values (Figs \ref{fig:SM_WB_env}, \ref{fig:SM_WB_env2}, \ref{fig:SM_UNdata} ) forced us to abandon many indicators.
It also questions whether the indicators currently used are good proxies of progress, or lack of it, in the SDG agenda.
...
Leveraging these subnational data, future research could explore whether the patterns here reported hold at finer scales.
...
Preliminary analysis of subnational inequality shows that different trends and mechanisms could be at place, calling for different policy interventions depending on context [@chrisendo2024].
Hence testing for mechanisms would benefit from a higher resolution in space and time that we were not able to test with SDG data.
...
We also find preliminary evidence for hysteresis, however the lack of transitions between regimes pervade us of empirically estimating potential tipping points.
...
Further work can look into this hypothesis by looking into other datasets or longer time series, or by exploring the plausibility of alternative mechanisms through modelling [@molander2022].
Generative models for wealth inequality exist inspired in biological mechanisms of heredity [@arrow2009], but less is known on inequality mechanisms for income or gender dimensions, although some hypothesis have been put forward [@chancel2020unsustainable; @molander2022].

# Conclusion

Inequality is one of the greatest challenges of our time.
We explored potential synergies and trade-offs between inequality and biodiversity related sustainable development goals.
We confirm some of the synergies and trade-offs previously reported, but also showed that data gaps, low quality and low variability pervade the ability of countries to measure progress in a meaningful way towards the SDG agenda.
We discussed alternative datasets that could improve independent monitoring, and suggest further studies investigating whether the patterns here reported hold at subnational scales.
We found support for the hypothesis of inequality regimes and hysteresis.
Preliminary, we showed that corruption can drive countries between two regimes of inequality.
Further testing this theory requires higher resolution data closer to the scale at which generative mechanisms are hypothesized.

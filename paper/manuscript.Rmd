...
We focused subsequent analysis only on country level indicators for SDGs 10, 14 and 15, related to inequality and the biosphere respectively. We complemented the SDGs datasets with inequality data from the World Inequality database (WID), using their estimates of the pre-tax national income computed as ratio or Gini.
...
Missing values were then imputed using a cubic spline, leaving us with `r df_dat |> pull(country_code) |> unique() |> length()` countries, `r df_dat |> pull(year) |> unique() |> length()` years of data across 9 indicators for the WB dataset; and `r df_un_reduced |> pull(series_name) |> unique() |> length()` indicators, `r df_un_reduced |> pull(iso3) |> unique() |> length()` countries over `r df_un_reduced |> pull(year) |> unique() |> length()` years for the UN dataset. The UN dataset was further reduced to 50 countries because the WID does not report inequality timeseries for Fiji.

*Ordination:* We used multiple factor analysis and principal component analysis to reduce the dimensionality of the data and explore similarities and differences across countries. 
...
\begin{figure*}[ht]
\centering
\includegraphics[width = 6in, height = 6in]{WB_environment_SDGs}

...
#save(key, file = "data/wb_var_defs.Rda")
```
...
q <- dat |>
    ...
    scale_fill_viridis_c(
        ...
        guide = guide_colorbar(barwidth = unit(2,"cm"), barheight = unit(5,"mm"))) + 
...
p/q
# ggsave(
#     plot = p/q, filename = "WB_selected_vars.png", path = "paper/figures/", device = "png",
#     width = 7.5, height = 7, dpi = 400, bg = "white"
# )
...
## correct Namibia is not a NA, but the country code is NA
ineq_dat <- ineq_dat |> 
    mutate(country = case_when(is.na(country) ~ "NA", TRUE ~ country)) #|> filter(country == "NA")
```
`m` is total, `a` is average. All other codes are documented here: [https://wid.world/codes-dictionary/#packages](https://wid.world/codes-dictionary/#packages). The selected variables are:

- `rptinc992j_p0p100` which means the ratio of the pretax national income adults (pop > 20yrs) of the top 10% / the bottom 50%

- `sptinc992j_p99p100`: share of pretax national income adults equal-split for the top 1% 

- `gptinc992j_p0p100`: Gini coefficient for the pretax national income for the adult population
...
# ggsave(
#     plot = crg, filename = "WB_correlogram.png", path = "paper/figures/", device = "png",
...
# ggsave(
#     plot = a/b/c + plot_layout(heights = c(1,1,1.5)), 
#     filename = "UN_selected_vars.png", path = "paper/figures/", device = "png",
#     width = 7, height = 8, dpi = 400, bg = "white"
# )
...
p2 <- sdg |> 
   ...
   # group_by(indicator, indicator_name, iso3) |> 
    ...
    mutate(series_name = str_replace(series_name, " covered", "\ncovered"),
           series_name = str_replace(series_name, " certification", "\ncertification")) |> 
...
# ggsave(
#     plot = p2/q2, filename = "UN_reduced_vars.png", 
#     path = "paper/figures/", device = "png",
#     width = 7, height = 7.5, dpi = 400, bg = "white"
# )
...
# ggsave(
#     plot = crg, filename = "UN_correlogram.png", path = "paper/figures/", device = "png",
#     width = 7, height = 7, dpi = 400, bg = "white"
# )
```
...
df_un_reduced <- df_un_reduced |> ungroup() |> #group_by(iso3)
    ...
    filter(iso3 != "FJI") |> # drop out Fiji, it does not exist in ineq data and creates NAs
    ...
    left_join(
        ineq_dat |> 
            ...
            filter(series_name != "sptinc992j_p0p100") |> # all ones, no variability
            right_join(
                countries |> 
...
pca1$rotation |> dim() # 228 variables*years, 150 PCs because there was 150 countries after filtering missing values in rptinc992j_p0p100.
pca1$x |> dim() # The loadings of 150 countries in each of the 150 components.
pca1$center |> length() # 228 variable centers
pca1$scale |> length() # 228 variable scales.
```
...
p <- pca1$rotation |> 
    ...
    labs(y = "Component loadings", x = "Year", title = "Top 4 principal components",
         caption = "Data source: World Bank and World Inequality Database") +
    facet_wrap(~PC) + scale_color_brewer("Variables", palette = "Set3") +
    theme_light(base_size = 10)
p
# ggsave(
#     plot = p, file = "pca-wb-time.png", path = "paper/figures/", device = "png",
...
# ggsave(
#     filename = "pca_wb-data.png", path = "paper/figures/", dpi = 400, bg = "white",
...
**240220** To do: code categorical variables so they appear on the order of Inequality, life on land, life on water. Do the clustering routine on the PCA coordinates, use all components, or the first 10. Show the clusters on fig A.
...
**J231009** Now it's working, the problem was not the zero or low variance, the problem was NA induced by two countries that didn't match on the joints, Namibia whose iso2 is NA and got interpreted as missing value, and Fiji that does not exist on the WII database. So only 50 countries at the end.
...
```{r}
x <- summary(pca2, loadings = F, cutoff = 0.1)
x <- x$importance |> as_tibble(rownames = "var") |> 
    pivot_longer(cols = starts_with("PC"), names_to = "comp") |> 
    mutate(comp = str_remove(comp, "PC") |> as.numeric()) |> 
    dplyr::filter( comp <= 10 , var != "Standard deviation")
...
a <- pca2$x |> as_tibble() |> 
    select(PC1:PC2) |> 
    mutate(iso = df_un_reduced |> pull(iso3) |> unique() ) |> 
    ggplot(aes(PC1, PC2)) +
    geom_text(aes(label = iso), size = 2) +
    coord_equal() +
    theme_light(base_size = 6)
...
df_un_reduced |> 
    pivot_longer(3:last_col(), names_to = "variable", values_to = "value") |> 
    separate(variable, into = c("year", "indicator"), sep = "_") |> 
    skimr::skim() # 19 years, 9 variables


mfa1 <- df_un_reduced |> 
    select(-iso3, -country) |> 
    as.matrix(dimnames = list(df_un_reduced |> pull(iso3) |> unique())) |> 
    FactoMineR::MFA(group = rep(19, 9), type = rep("s", 9), ncp = 20) # 19 years times 9 variables, 20 components (PCAs)
```
...
```{r}
# df_dat$year |> unique() |> length() # 19yrs
...
mfa2 <- df_dat |> 
    ...
    ungroup() |> 
    select(-country_code) |> 
    as.matrix(dimnames = list(df_dat |> pull(country_code) |> unique())) |> 
    FactoMineR::MFA(group = rep(19, 12), type = rep("s", 12), ncp = 20) # 19 years times 9 variables
```
...
ggbiplot::ggbiplot(
    pca2,
    labels = df_un_reduced |> pull(iso3) |> unique(),
    circle = TRUE,
    varname.abbrev = TRUE)
```

Extract the weight per country on the relevant components to see how they move on PCA or MFA space.
...
```{r}
#save(pca1, pca2, mfa1, mfa2, file = "data/ordination_results.Rda")
```


```{r}
df_un_reduced |> 
    pivot_longer(3:last_col(), names_to = "variables", values_to = "values") |> 
    separate(variables, sep = "_", into = c("year", "variable")) |> 
    mutate(year= as.numeric(year)) |> 
    filter(variable == "gptinc992j") |> 
...
df_un_reduced |> 
    pivot_longer(3:last_col(), names_to = "variables", values_to = "values") |> 
    separate(variables, sep = "_", into = c("year", "variable")) |> 
    mutate(year= as.numeric(year)) |> #pull(variable) |> unique()
    mutate(variable = case_when(
        variable == "Average proportion of Freshwater Key Biodiversity Areas (KBAs) covered by protected areas (%)" ~ "freshwater",
        variable == "Average proportion of Marine Key Biodiversity Areas (KBAs) covered by protected areas (%)"~"marine",
        variable == "Average proportion of Mountain Key Biodiversity Areas (KBAs) covered by protected areas (%)" ~ "mountain",
        variable == "Average proportion of Terrestrial Key Biodiversity Areas (KBAs) covered by protected areas (%)" ~ "terrestrial", 
        variable == "Forest area certified under an independently verified certification scheme (thousands of hectares)" ~ "forest",
        variable == "gptinc992j" ~ "Gini",
        variable == "rptinc992j"~ "Ratio 10/50",
        variable == "sptinc992j" ~ "Share 1%",
        .default = variable
    )) |> 
    filter(variable %in% c("freshwater", "marine", "terrestrial", "mountain", "forest", "Gini", "Ratio 10/50", "Share 1%")) |> #pull(variable) |> unique()
    pivot_wider(names_from = variable, values_from = values) |> 
    ggplot(aes(Gini, freshwater)) + 
    geom_line(aes(group = iso3, color = year))
...
- Clustering analysis on the reduced dimensions (PCA, MFA)
- Plot ineq against biodiv proxies, does the path foward differ from the path backwards = hysteresis?

...
pca1$x |> as_tibble() |> 
    select(PC1:PC2) |> 
    mutate(iso = df_dat$country_code |> unique()) |> 
    ggplot(aes(PC1, PC2)) +
    geom_text(aes(label = iso), size = 2) +
    geom_segment(
        data = pca1$rotation |>
            as_tibble(rownames = "factor") |>
            separate(factor, into = c("year", "factor")),
        aes(xend = PC1, yend = PC2, x= 0, y = 0, color = factor, alpha = year)) +
    coord_equal()
...
```{r}
pca2 <- df_un_reduced |> 
    unite(col = "var", series_name, year, sep = "_") |> 

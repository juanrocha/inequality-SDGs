...
df_summary |> 
    ...
    mutate(country = fct_reorder2(country, magnitude, Gini)) |> 
    pivot_longer(cols = sd_magnitude:gamma, names_to = "vars", values_to = "vals") |> 
...
pc1 <- ggbiplot::ggbiplot(pc, groups = as.character(stab@clusterObjs$pam$`2`$clustering),
         ellipse = TRUE, labels = df_summary$country_code, labels.size = 2.5,
         varname.size = 4) +
    scale_color_manual("Groups", values = c("orange", "purple")) +
...
Note to self: It does not make sense to plot the average distance traversed in the space per country. Take Luxemburg as example, it started and ended verly close by. The absolute distance is what I'm calculating with the euclidian distance on that plane, then the first-diff values are just increases or decreases with respect to initial conditions. Averages are then misleading, because on average Luxemburg moved a lot (in circles). 
...
a1 <- df_dat |> 
    ...
    mutate(Group = as.character(stab@clusterObjs$pam$`2`$clustering)) |> 
    left_join(
        dat |> select(country, country_code) |> 
            filter(!is.na(country_code)) |> unique()) |> 
    ggplot(aes(EF, Gini)) +
    geom_segment(
        ...
        arrow = arrow(length = unit(0.15, "cm")), size = 0.15) +
    geom_text(
        aes(x = EF *1.1, y = Gini * 1.1, label = country_code, colour = Group), size = 2) +
    scale_color_manual("Groups", values = c("orange", "purple")) +
...
a2 <- df_dat |> 
    ...
    geom_text(aes(x = GNI * 1.1, y = Gini *1.1, 
                  label = country_code, colour = Group), size = 2) +
    scale_color_manual("Groups", values = c("orange", "purple")) +
    theme_light(base_size = 6) +
    theme(panel.grid = element_blank(), legend.position = c(0.8,0.2)) 

c2 + annotation_custom(
        grob = ggplotGrob(a2),
        xmin = 1e+01, xmax = 1e+03, ymin = 40, ymax = 60
    ) 

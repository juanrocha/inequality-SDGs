...
pc1 <- ggbiplot::ggbiplot(
    ...
    ellipse = TRUE, labels = df_summary$country_code, labels.size = 1.5,
    varname.size = 2) +
...
df_all <- gini |> left_join(ef) |> left_join(gni)
...
```{r}
pc_all <- prcomp(df_all |> 
                 select(where(is.numeric)) ,
             center = TRUE, scale. = TRUE)


pc2 <- ggbiplot::ggbiplot(
    pc_all, groups = as.character(stab@clusterObjs$pam$`2`$clustering ),
    ellipse = TRUE, labels = df_all$country_code, labels.size = 1.5,
    varname.size = 2) +
...
ef_map <-  world |>
    ...
    scico::scale_fill_scico(
        "Mean Ecological Footprint",palette = "romaO",
...
gni_map <-  world |>
    ...
    labs(tag = "C") +
    theme_void(base_size = 6) +
    theme(legend.position = "bottom", legend.key.height = grid::unit(2, "mm"), 
